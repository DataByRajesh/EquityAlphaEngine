...
def _parse_database_url(database_url: str) -> dict:
    ...
    return {
        'driver': parsed.scheme.split('+')[-1] if '+' in parsed.scheme else 'postgresql',
...
def _get_cloud_sql_instance_name():
    ...
    try:
        ...
        logger.warning("Could not determine Cloud SQL instance name, using direct connection")
        return None

def _create_engine_with_retry(database_url: str, parsed: dict, use_connector: bool = False, instance_name: str = None):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if attempt < MAX_RETRIES - 1:
                logger.warning(f"Engine creation attempt {attempt + 1} failed: {e}. Retrying in {RETRY_DELAY} seconds...")
                time.sleep(RETRY_DELAY)
            else:
                logger.error(f"All {MAX_RETRIES} engine creation attempts failed. Last error: {e}")
    
    raise RuntimeError(f"Failed to create SQLAlchemy engine after {MAX_RETRIES} attempts: {last_exception}")
...
def initialize_engine():
    ...
    if _is_cloud_sql_host(host):
        ...
        if instance_name:
            try:
                return _create_engine_with_retry(database_url, parsed, use_connector=True, instance_name=instance_name)
            except Exception as e:
                logger.warning(f"Failed to create engine with Cloud SQL connector: {e}. Falling back to direct connection.")
...
def reinitialize_engine(new_database_url=None):
    ...
    # Check if we should use Cloud SQL connector
    if _is_cloud_sql_host(host):
        logger.info("Detected Cloud SQL host for reinitialization, attempting to use Cloud SQL connector")
        instance_name = _get_cloud_sql_instance_name()
        if instance_name:
            try:
                engine = _create_engine_with_retry(database_url, parsed, use_connector=True, instance_name=instance_name)
                SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)
                logger.info("Engine reinitialized successfully with Cloud SQL connector.")
                return
            except Exception as e:
                logger.warning(f"Failed to reinitialize engine with Cloud SQL connector: {e}. Falling back to direct connection.")
